...
then
  ...
  cd /data/openpilot.$2 
  /data/data/com.termux/files/usr/bin/git checkout apk/ai.comma.plus.frame.apk

  now=`date +"%Y/%m/%d %T"`
  echo "[ $now ] Switching branch $3"
  /data/data/com.termux/files/usr/bin/git checkout $3
  echo "[ $now ] Copying APK to /data/openpilot.$2/apk/"
  cp /data/communitypilot_scripts/ai.comma.plus.frame.apk /data/openpilot.$2/apk/
